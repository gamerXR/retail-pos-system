    ...
    try {
      ...
      const emailBody = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <p>Please find attached the sales summary report for ${dateFrom} to ${dateTo}.</p>
          ...
          <pre style="background: #f9f9f9; padding: 15px; border-radius: 5px; overflow-x: auto;">${csvContent}</pre>
          <hr style="margin: 20px 0; border: none; border-top: 1px solid #ddd;">
      ...
      if (emailResult.success) {
        ...
        return {
          success: false,
          message: emailResult.message || "Failed to send email. Please check the email address and try again."
        };
      }
    } catch (error) {
